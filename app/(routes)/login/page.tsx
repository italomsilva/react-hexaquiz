...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(
        `Ocorreu um erro ao tentar ${isRegisterMode ? "cadastrar" : "entrar"}`,
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const handleResetPassword = (e: React.FormEvent) => {
    ...
    const subject = encodeURIComponent(
      "Solicitação de Redefinição de Senha - HexaQuiz",
    );
  ...
  return (
    <div
      ...
      style={{
        ...
        backgroundColor:
             ...
             "rgba(0, 32, 34, 0.1)",
        backgroundBlendMode: theme === "dark" ? "darken" : "overlay",
    ...
    >
      <main className="min-h-[100dvh] flex flex-col items-center justify-center p-6 relative overflow-y-auto overflow-x-hidden">
        <div className="w-full max-w-sm z-10 flex flex-col items-center space-y-10 my-4">
          <div className="text-center flex flex-col items-center space-y-3">
            <h1 className="text-5xl font-black italic tracking-tighter text-primary">
              HEXA<span className="text-white">QUIZ</span>
            </h1>
          ...
          <form
            ...
            className="w-full space-y-5 bg-background border border-border-subtle p-8 rounded-2xl shadow-2xl transition-all duration-500 overflow-hidden"
          >
            {/* Alternador de Modo (Switcher) */}
            {!isResetPasswordMode && (
              <div className="flex p-1 bg-surface border border-border-subtle rounded-xl mb-6">
                <button
                  type="button"
                  onClick={() => {
            ...
            <div
              className={`space-y-5 transition-all duration-300 ${isResetPasswordMode ? "hidden" : "block"}`}
            >
              {isRegisterMode && (
            ...
            {!isResetPasswordMode ? (
              ...
              <div className="space-y-4 pt-4 animate-slide-down">
                <p className="text-sm text-gray-400 text-center mb-4">
                  Envie seus dados para os desenvolvedores solicitando a
